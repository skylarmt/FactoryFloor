...
if (count($tasks) > 0) {
    foreach ($tasks as $task) {
        ...
        . "<button type='submit' class='btn btn-primary'><i class='fa fa-play'></i> Start Task</button>"
        . "</form>"
        . "<form action='task.php' method='POST' class='form-inline' style='display: inline-block; padding-left: 5px;'>"
        . "<input type='hidden' name='taskid' value='" . $task['taskid'] . "' />"
        . "<input type='hidden' name='action' value='finish' />"
        . "<button type='submit' class='btn btn-success'><i class='fa fa-stop'></i> Finish Task</button>"
        . "</form>"
        . "</div>"
        . "</div>"
        . "</div>"
